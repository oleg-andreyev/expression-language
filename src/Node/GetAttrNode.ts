...
export default class GetAttrNode extends Node {
    ...
    evaluate(functions, values) {
        switch(this.attributes.type) {
            case GetAttrNode.PROPERTY_CALL:
                const obj = this.nodes.node.evaluate(functions, values),
                ...
                return obj[property];
            case GetAttrNode.METHOD_CALL:
                const obj2 = this.nodes.node.evaluate(functions, values),
                ...
                const evaluatedArgs = this.nodes.arguments.evaluate(functions, values);
                return obj2[method].apply(null, evaluatedArgs);
            case GetAttrNode.ARRAY_CALL:
                const array = this.nodes.node.evaluate(functions, values);
