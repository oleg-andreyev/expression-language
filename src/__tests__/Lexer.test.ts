...
test('tokenize', () => {
    ...
    for (const tokenizeData of data) {
        const tokens = tokenizeData[0] as Token[];
        const expression = tokenizeData[1];

        tokens.push(new Token(Token.EOF_TYPE, null, expression.length + 1));

        //console.log("Testing: ", expression);

        const generatedStream = tokenize(expression);
        const expectedStream = new TokenStream(expression, tokens);
