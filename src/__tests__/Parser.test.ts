...
test('parse with invalid postfix data', () => {
    const invalidPostfixData = getInvalidPostfixData();
    for (const oneTest of invalidPostfixData) {
        try {
            const parser = new Parser();
            parser.parse(tokenize(oneTest[0]), oneTest[1] as string[]);
            console.log("The parser should throw an error.");
            expect(true).toBe(false); // This should fail
        } catch (err) {
...
test('parse', () => {
    ...
    for (const parseDatum of parseData) {
        //console.log("Testing ", parseDatum[1], parseDatum[2]);

        const parser = new Parser();
        const generated = parser.parse(tokenize(parseDatum[1]), parseDatum[2] as string[]);
