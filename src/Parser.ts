...
export default class Parser {
    ...
    private names: any[];
    private objectMatches: Record<string, number>;
    private cachedNames: string[] = [];
    private nestedExecutions: number;
    ...
    parse(tokenStream: TokenStream, names = []) {
    ...
    parseExpression(precedence = 0): Node {
        ...
        && this.binaryOperators[token.value].precedence >= precedence) {
            ...
            const op = this.binaryOperators[token.value];
            this.tokenStream.next();

            const expr1 = this.parseExpression(OPERATOR_LEFT === op.associativity ? op.precedence + 1 : op.precedence);
    ...
    getPrimary() {
        ...
        if (token.test(Token.OPERATOR_TYPE)
            ...
            const operator = this.unaryOperators[token.value];
            this.tokenStream.next();
        ...
        if (this.names && this.names.length > 0) {
            ...
            for (const name of this.names) {
                if (typeof name === "object") {
                    ...
                    this.objectMatches[values[0] as string] = index;
    ...
    parsePostfixExpression(node) {
        ...
        while (Token.PUNCTUATION_TYPE === token.type) {
            if ('.' === token.value) {
                ...
                const arg = new ConstantNode(token.value, true);
                const _arguments = new ArgumentsNode();
                let type = null;

                if (this.tokenStream.current.test(Token.PUNCTUATION_TYPE, "(")) {
                    type = GetAttrNode.METHOD_CALL;
                    for (const n of Object.values(this.parseArguments().nodes)) {
    ...
    parseArguments() {
        ...
        return new Node(args as Record<number, Node>);
    }
}
