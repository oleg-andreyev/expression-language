        ...
        if (number !== null) {
            ...
            if ('([{'.indexOf(expression[cursor]) >= 0) {
                ...
                if (')]}'.indexOf(expression[cursor]) >= 0) {
                    ...
                    const str = extractString(expression.substr(cursor));
                    if (str !== null) {
                        ...
                        const operator = extractOperator(expression.substr(cursor));
                        if (operator) {
                            ...
                            if (".,?:".indexOf(expression[cursor]) >= 0) {
                                ...
                                const name = extractName(expression.substr(cursor));
                                if (name) {
...
function extractName(str) {
    ...
    const matches = str.match(/^[a-zA-Z_\x7f-\xff][a-zA-Z0-9_\x7f-\xff]*/);
    if (matches && matches.length > 0) {
        extracted = matches[0];
    }
