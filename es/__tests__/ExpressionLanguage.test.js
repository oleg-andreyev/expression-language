...
test('parse throws', function () {
    var exprLang = new ExpressionLanguage();
    try {
...
test('caching for overridden variable names', function () {
    var expressionLanguage = new ExpressionLanguage(), expression = 'a + b';
...
test('strict equality', function () {
    var expressionLanguage = new ExpressionLanguage(), expression = '123 === a';
...
function getRegisterCallbacks() {
    var provider = {
        getFunctions: function () {
            return [
                new ExpressionFunction('fn', function () {
                }, function () {
                })
            ];
        }
    ...
    return [
        ...
        [
            function (expressionLanguage) {
                expressionLanguage.addFunction(new ExpressionFunction('fn', function () {
                }, function () {
                }));
            }
        ],
...
test('evaluate', function () {
    ...
    for (var _i = 0, evaluateData_1 = evaluateData; _i < evaluateData_1.length; _i++) {
        ...
        var expressionLanguage = new ExpressionLanguage(), provider = evaluateDatum[3], expression = evaluateDatum[0], values = evaluateDatum[1], expectedOutcome = evaluateDatum[2];
        if (provider) {
            expressionLanguage.registerProvider(provider);
        }
