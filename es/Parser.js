...
export var OPERATOR_LEFT = 1;
export var OPERATOR_RIGHT = 2;
var Parser = /** @class */ (function () {
    ...
    Parser.prototype.parse = function (tokenStream, names) {
        ...
        if (!this.tokenStream.isEOF()) {
            throw new SyntaxError("Unexpected token \"".concat(this.tokenStream.current.type, "\" of value \"").concat(this.tokenStream.current.value, "\"."), this.tokenStream.current.cursor, this.tokenStream.expression);
        }
        return node;
    };
    Parser.prototype.parseExpression = function (precedence) {
        ...
        while (token.test(Token.OPERATOR_TYPE)
            ...
            var expr1 = this.parseExpression(OPERATOR_LEFT === op.associativity ? op.precedence + 1 : op.precedence);
        ...
        switch (token.type) {
                    ...
                    default:
                        if ("(" === this.tokenStream.current.value) {
                            if (this.functions[token.value] === undefined) {
                                throw new SyntaxError("The function \"".concat(token.value, "\" does not exist"), token.cursor, this.tokenStream.expression, token.value, Object.keys(this.functions));
                            }
                            node = new FunctionNode(token.value, this.parseArguments());
                        }
                        else {
                            if (!this.hasVariable(token.value)) {
                                throw new SyntaxError("Variable \"".concat(token.value, "\" is not valid"), token.cursor, this.tokenStream.expression, token.value, this.getNames());
                            }
            ...
            default:
                if (token.test(Token.PUNCTUATION_TYPE, "[")) {
                    ...
                    throw new SyntaxError("Unexpected token \"".concat(token.type, "\" of value \"").concat(token.value, "\""), token.cursor, this.tokenStream.expression);
                }
        }
    ...
    Parser.prototype.parseHashExpression = function () {
        ...
        while (!this.tokenStream.current.test(Token.PUNCTUATION_TYPE, '}')) {
            ...
            if (this.tokenStream.current.test(Token.STRING_TYPE)
                ...
                throw new SyntaxError("A hash key must be a quoted string, a number, a name, or an expression enclosed in parentheses (unexpected token \"".concat(current.type, "\" of value \"").concat(current.value, "\""), current.cursor, this.tokenStream.expression);
            }
    ...
    Parser.prototype.parsePostfixExpression = function (node) {
        ...
        while (Token.PUNCTUATION_TYPE === token.type) {
            if ('.' === token.value) {
                ...
                var arg = new ConstantNode(token.value, true);
                var _arguments = new ArgumentsNode();
                var type = null;
                if (this.tokenStream.current.test(Token.PUNCTUATION_TYPE, "(")) {
